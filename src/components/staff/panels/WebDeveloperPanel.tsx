...
import { Code, ClipboardList, CheckCircle2, Clock, Loader2 } from "lucide-react";
...
export default function WebDeveloperPanel() {
  const { tasks, loading, refetch } = useStaffTasks("web_developer");
    ...
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex items-center gap-3">
          <div className="h-11 w-11 rounded-xl bg-gradient-to-br from-cyan-500/20 to-blue-500/20 flex items-center justify-center">
            <Code className="h-5.5 w-5.5 text-cyan-600" />
          </div>
          <div>
            <h1 className="text-xl font-bold text-foreground">ওয়েব ডেভেলপার</h1>
            <p className="text-[11px] text-muted-foreground">আপনার ডেভেলপমেন্ট টাস্ক ও প্রজেক্ট</p>
          </div>
        </div>
      </motion.div>
      ...
      <div className="space-y-3">
        {loading ? (
          ...
          <div className="text-center py-12 rounded-xl border border-border bg-card">
            <Code className="h-10 w-10 text-muted-foreground/20 mx-auto mb-2" />
