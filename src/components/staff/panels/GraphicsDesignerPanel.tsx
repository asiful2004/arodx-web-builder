...
import { Palette, ClipboardList, CheckCircle2, Clock, Loader2 } from "lucide-react";
import { useStaffTasks, TASK_STATUS_CONFIG, type TaskStatus } from "@/hooks/useStaffTasks";
    ...
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex items-center gap-3">
          <div className="h-11 w-11 rounded-xl bg-gradient-to-br from-pink-500/20 to-rose-500/20 flex items-center justify-center">
            <Palette className="h-5.5 w-5.5 text-pink-600" />
          </div>
          <div>
            <h1 className="text-xl font-bold text-foreground">গ্রাফিক্স ডিজাইনার</h1>
            <p className="text-[11px] text-muted-foreground">আপনার ডিজাইন টাস্ক ও প্রজেক্ট</p>
          </div>
        </div>
      </motion.div>
      ...
      <div className="flex gap-1.5 flex-wrap">
        {[
          ...
          <Button key={f.value} variant={statusFilter === f.value ? "default" : "outline"} size="sm" className="h-7 text-[10px] px-2.5" onClick={() => setStatusFilter(f.value)}>
            {f.label}
          </Button>
        ))}
      </div>
      ...
      <div className="space-y-3">
        {loading ? (
          ...
          <div className="text-center py-12 rounded-xl border border-border bg-card">
            <Palette className="h-10 w-10 text-muted-foreground/20 mx-auto mb-2" />
            <p className="text-sm text-muted-foreground">{statusFilter === "all" ? "এখনো কোনো টাস্ক অ্যাসাইন হয়নি" : "এই ফিল্টারে কোনো টাস্ক নেই"}</p>
          </div>
        ) : (
          filtered.map((task) => (
            <TaskCard key={task.id} task={task} canManage={false} onRefetch={refetch} />
          ))
        )}
      </div>
    </div>
