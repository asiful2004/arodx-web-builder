...
const SignIn = () => {
  ...
  // QR login: user scans QR on a logged-in device to log in here
  ...
  useState(() => {
    // will be triggered by useEffect below
  });

  // Generate QR on mode switch
  const handleSwitchToQr = () => {
    setLoginMode("qr");
    if (!qrLoginToken && !qrLoginWaiting) {
      generateQrLogin();
    }
  };
  ...
  // QR countdown
  if (qrLoginWaiting && !qrLoginExpired && qrTimeLeft > 0) {
    // We need a timer - using a simple approach
  }

  // Listen for QR approval
      ...
      >
        <div className="bg-card/60 backdrop-blur-xl border border-border rounded-2xl p-8">
          ...
          {loginMode === "qr" && (
            <QrLoginPanel
              token={qrLoginToken}
              waiting={qrLoginWaiting}
              expired={qrLoginExpired}
              timeLeft={qrTimeLeft}
              loading={loading}
              onRetry={() => {
              ...
              onSetTimeLeft={setQrTimeLeft}
              onSetExpired={setQrLoginExpired}
              onSetWaiting={setQrLoginWaiting}
              onSetToken={setQrLoginToken}
...
interface QrLoginPanelProps {
  ...
  loading: boolean;
  onRetry: () => void;
  onSetTimeLeft: (fn: (prev: number) => number) => void;
  onSetExpired: (val: boolean) => void;
  onSetWaiting: (val: boolean) => void;
  onSetToken: (val: string | null) => void;
...
function QrLoginPanel({
  ...
  }, [waiting, expired, onSetTimeLeft, onSetExpired, onSetWaiting]);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "UPDATE", schema: "public", table: "device_login_requests", filter: `token=eq.${token}` },
        async (payload) => {
  ...
  }, [token, waiting, onSetWaiting, onSetToken, onSetExpired, onLoginSuccess]);
    ...
    <div className="text-center space-y-4">
      {loading && !token ? (
        ...
        <div className="space-y-4 py-4">
          ...
          <Button onClick={onRetry} disabled={loading} className="gap-2">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-1">
              ...
              <Progress value={(timeLeft / 300) * 100} className="h-1 w-32 mx-auto" />
            </div>
          </div>
        </div>
      ) : null}
